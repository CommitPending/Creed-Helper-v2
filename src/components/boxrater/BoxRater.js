    ...
    try {
      ...
      if (data.success) {
        ...
        result.pokemon.forEach((poke) => {
          if (
            poke.loan === '0' &&
            coloreds.some((color) => poke.name.startsWith(color))
          ) {
            findrates.add(poke.name.toLowerCase())

            if (poke.level === 5) {
              pokemonList.base.push({
                name: poke.name,
                gender: poke.gender,
                level: poke.level,
              })
            } else if (poke.level > 5) {
              pokemonList.unbase.push({
                name: poke.name,
                gender: poke.gender,
                level: poke.level,
              })
            } else {
              pokemonList.other.push({
                name: poke.name,
                gender: poke.gender,
                level: poke.level,
              })
        ...
        if (
          pokemonList.base.length +
            pokemonList.unbase.length +
            pokemonList.other.length ===
          0
        ) {
    ...
    <div>
      <Card>
        <CardBody>
          ...
          <Label>
            <p>
              <strong>NOTE:</strong> Unbase: 0.8x Rate List | Level 4 or less:
              3x Rate List | Genderless/Special Genders are rated normally.
            </p>
          </Label>
        </CardBody>
      </Card>
